...
import { Post, UpdateProps } from "shared/src/Type";
...
const Detail: React.FC = () => {
  ...
  const downloadHandler = (fileid: number) => {
    const download = async () => {
      const response = await axios.get(
        `${baseUrl}/api/notice/download/${fileid}`,
        {
          headers: {
        ...
        <div className="mt-[20px]">
          ...
          {boardType == "notice" ? (
            <div className="mb-4">
              ...
              >
                <button>{post.files[0]?.originalFilename}</button>
              </div>
            </div>
          ) : (
            <></>
  ...
  const postDelete = () => {
    ...
    if (selection) {
      simpleDelete(cookies.Token, `api/${boardType}/delete/${id}`).then(() => {
        alert("게시글이 삭제되었습니다.");
